...
// Return an map of file that has changed in a folder
...
function packageUpdatesSince( ref, repoDir, packageDir ) {
    ...
    .then(function( exists ) {
        ...
        .then(function( files ){
            ...
            return Q.nfcall( fs.appendFile, args.outputFile, '[' )
            .then(function(){
                // do a cat for every file and write it to the feed
                return Q.all( files.map(function( file ){
                    return exec( ".", 'CatFile', { file: file, repoDir: repoDir })
                    .then(function( fileContent ){
                        var fileContent = fileContent && fileContent[0];
                        // Check if file has content before writing to file
                        if ( fileContent && fileContent[0] && fileContent[0] !== ""){
                            // parse the json in file and construct feed item entry
                            var jsonContent = JSON.parse( fileContent );
                            var feedItem = {
                                content:    jsonContent.content,
                                jekyllurl:  jsonContent.url
                            }
                            // write to item to file attaching a , at the end
                            return Q.nfcall( fs.appendFile, args.outputFile, JSON.stringify( fileContent ) + ',' )
                        }
                    })
                }))
            })
