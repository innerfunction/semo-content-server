...
var Commands = {
    'CloneGitHub':              "git clone https://{username}:{password}@github.com/{username}/{feed}.git",
    'CloneGit':                 "git clone https://{username}:{password}@g{server}/{username}/{feed}.git",
 ...
 * @param feed: name of the repo in GitHub
 * @param username/pass: GitHub username and password.
 * @param beseReposDir: the
 ``*/
// TODO: support any Git server, not just GitHub
function clone( baseReposDir, username, password, feed ){
    // File are placed in: /reposDir/username/repoName
    var repoDir = path.join( baseReposDir, username );
    ...
    .then(function( repoExist ){
        if( repoExist ) {
            ...
            .then(function(){
                var args =  { username: username, password: password, feed: feed };
                return exec( repoDir, 'CloneGitHub' , args);
            })
            .then(function( msg ){
...
function packageUpdatesSince( ref, repoDir, packageDir ) {
    ...
    .then(function( exists ) {
        ...
        .then(function() {
            ...
            // reference commit.
            return exec( repoDir, 'ListTrackedFilesAtCommit', { commit: ref });
        })
        .then(function( files ) {
        ...
        .then(function( fileset ) {
            ...
            return exec( repoDir, 'ListTrackedFilesAtCommit', { commit: args.current })
