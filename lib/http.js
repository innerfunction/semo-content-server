...
function makeZipURL( settings, feedFile ) {
    feedFile = mods.path.relative( settings.packageDir, feedFile );
    return settings.zipBaseURL+feedFile;
}
...
function updateSinceResponse( settings, res, feed, since, result ) {
    var url = makeZipURL( settings, result.feedFile );
    sendJSON( res, {
        status:     'update-since',
        feed:       feed,
        since:      since,
                    ...
                    .then(function( hash ) {
                        ...
                        .fail(function( err ) {
                            ...
                            if( /fatal: Not a valid object name/.test( err ) ) {
                                // This will occur if the specified commit folder output-json is not in commit
                                returnCurrentContent( repoDir, packageDir, settings, res, feed );
                            }
